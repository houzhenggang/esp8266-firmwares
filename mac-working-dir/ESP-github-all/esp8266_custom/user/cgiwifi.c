...
int ICACHE_FLASH_ATTR cgiWiFiScan(HttpdConnData *connData) {
	...
	if (cgiWifiAps.scanInProgress==1) {
		...
		for (i=0; i<cgiWifiAps.noAps; i++) {
			os_strcpy(strssid, (char*)stconf.ssid);
			if (os_strcmp(strssid, cgiWifiAps.apData[i]->ssid)==0) {
				wifi_get_ip_info(STATION_IF, &ipconfig);
				os_sprintf(ipAddress, IPSTR, IP2STR(&ipconfig.ip));
				//os_printf("IP : "IPSTR, IP2STR(&ipconfig.ip));
			} else {
				os_sprintf(ipAddress, " ");			
			}
			len=os_sprintf(buff, "{\"essid\": \"%s\", \"rssi\": \"%d\", \"enc\": \"%d\", \"ipaddr\": \"%s\"}%s\n", 
					cgiWifiAps.apData[i]->ssid, cgiWifiAps.apData[i]->rssi, 
					cgiWifiAps.apData[i]->enc, ipAddress, (i==cgiWifiAps.noAps-1)?"":",");
			espconn_sent(connData->conn, (uint8 *)buff, len);
		}
		len=os_sprintf(buff, "]\n}\n}\n");
		espconn_sent(connData->conn, (uint8 *)buff, len);
...
#endif
    os_printf("Connecting to station %s with password %s\n", essid, passwd);
	return HTTPD_CGI_DONE;
}
