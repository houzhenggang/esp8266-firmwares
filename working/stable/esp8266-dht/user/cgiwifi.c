...
void ICACHE_FLASH_ATTR tplWlan(HttpdConnData *connData, char *token, void **arg) {
	...
	if (os_strcmp(token, "WiFiMode")==0) {
		x=wifi_get_opmode();
		if (x==1) os_strcpy(buff, "standalone client");
		if (x==2) os_strcpy(buff, "soft access point");
		if (x==3) os_strcpy(buff, "client + access point");
	...
	} else if (os_strcmp(token, "currIp")==0) {
        wifi_get_ip_info(STATION_IF, &ipconfig);
        //os_strcpy(buff, (char*)&ipconfig.ip.addr);
        os_sprintf(buf, IPSTR, IP2STR(&ipconfig.ip));
        os_strcpy(buff, (char*)buf);
